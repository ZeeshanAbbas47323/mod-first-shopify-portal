...
function getColumns(
  onToggleStatus: (row: ColorRow, next: boolean) => Promise<void>
): ColumnDef<ColorRow>[] {
  return [
  {
    accessorKey: "name",
...
const COLUMN_FILTERS: Record<string, ColumnFilterDef> = {
  name: { type: "text", placeholder: "Search colors" },
...
export function ColorsSection() {
  ...
  const runExport = async (fileFormat: ExportFormat) => {
    setExportBusy(true);
    try {
      const exportRows = await fetchAllPages((page, limit) => listColors({ page, limit, filters: buildFilters() }), EXPORT_CAP);
      if (!exportRows.length) {
        toast.error("Nothing to export.");
        return;
      }
      await writeExport(fileFormat, "colors", [
  ...
  const handleToggleStatus = async (row: ColorRow, next: boolean) => {
    try {
      await updateRecordStatus("color", row.id, next);
      toast.success(next ? "Color activated." : "Color deactivated.");
      setRefreshKey((k) => k + 1);
    } catch (error) {
